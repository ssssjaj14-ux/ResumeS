...
// Generate more SEO-optimized jobs
const generateMoreJobs = (): Job[] => {
  ...
  for (let i = 6; i <= 1000; i++) {
    ...
    const experience = ['0-2 years', '1-3 years', '2-4 years', '3-5 years', '5-8 years'][Math.floor(Math.random() * 5)];
    const salary = ['₹3-8 LPA', '₹8-15 LPA', '₹15-25 LPA', '₹25-40 LPA', '₹40-60 LPA'][Math.floor(Math.random() * 5)];
    const remote = Math.random() > 0.6;
    ...
    additionalJobs.push({
      ...
      experience,
      salary,
      description: `Join ${company} as a ${title} and work on exciting projects that impact millions of users. We're looking for passionate professionals who want to make a difference in the tech industry.`,
...
// Enhanced search with real-time capabilities
export const searchJobs = async (query: string, filters: {
  ...
  return filteredJobs.slice(0, 50); // Return top 50 results
};
...
const calculateRelevanceScore = (job: Job, searchTerms: string[]): number => {
  ...
  searchTerms.forEach(term => {
    ...
    if (job.skills.some(skill => skill.toLowerCase().includes(term))) score += 8;
    // Description matches get low score
    if (job.description.toLowerCase().includes(term)) score += 2;
  });
...
// Enhanced RapidAPI integration with better error handling
export async function fetchJobsFromRapidAPI({ jobTitle, location, locationType = 'ANY', yearsOfExperience = 'ALL' }: {
  ...
  const url = `https://jsearch.p.rapidapi.com/search?query=${encodeURIComponent(jobTitle + ' ' + location)}&page=1&num_pages=3&date_posted=all&remote_jobs_only=false&employment_types=FULLTIME%2CPARTTIME%2CCONTRACTOR%2CINTERN&job_requirements=under_3_years_experience%2Cmore_than_3_years_experience%2Cno_experience%2Cno_degree`;
...
// Enhanced smart job fetching with trending analysis
          ...
          for (const job of res.data.slice(0, batchSize)) {
            ...
            if (!seen.has(uniqueKey)) {
              ...
              const normalizedJob = {
                ...
                job_description: job.job_description || `Exciting opportunity for ${title} in ${location}`,
