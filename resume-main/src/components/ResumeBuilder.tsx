...
const templateComponents: Record<string, React.FC<any>> = {
  ...
  export2: ResumeTemplateExport2,
  export3: ResumeTemplateExport3,
  export4: ResumeTemplateExport4,
  export5: ResumeTemplateExport5,
  export6: ResumeTemplateExport6,
  export7: ResumeTemplateExport7,
};
...
const ResumeBuilder: React.FC = () => {
  ...
  const handleDownloadPDF = async () => {
    try {
      const templateKey = selectedTemplate.replace('.json', '').toLowerCase();
      const TemplateComponent = templateComponents[templateKey];
      if (!TemplateComponent) {
        toast.error('Selected template does not support PDF export.');
        return;
      }
      ...
      import('react-dom').then(ReactDOM => {
        ReactDOM.render(<TemplateComponent preview={false} />, container);
        setTimeout(async () => {
