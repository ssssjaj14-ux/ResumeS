...
import { Upload, FileText, CheckCircle, AlertCircle, X, Download, Brain } from 'lucide-react';
...
const ResumeImporter: React.FC<ResumeImporterProps> = ({ onDataImported, onClose }) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (file.type === 'application/pdf' || fileExtension === '.pdf') {
        setProcessingStep('Uploading PDF for extraction...');
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (file.type === 'text/plain') {
            ...
            // For DOC/DOCX files, try to extract text
  ...
  const extractDocText = async (buffer: ArrayBuffer): Promise<string> => {
    try {
      ...
      return text || 'No readable text found in document. Please use the "Paste Text" option.';
    } catch (error) {
      console.error('DOC extraction error:', error);
      return 'Error reading document file. Please use the "Paste Text" option.';
    }
  };
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                      {uploading ? processingStep : 'Drop your resume here'}
                    </h3>
                    ...
                    <p className="text-xs text-orange-600 dark:text-orange-400 mt-2">
                      💡 For PDF files: Use "Paste Text" option below
                    </p>
                  </div>
